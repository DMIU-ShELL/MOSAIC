...
from deep_rl.component.task import ParallelizedTask, ContinualWorld 
...
def global_config(config, name):
    ...
    config.state_normalizer = RunningStatsNormalizer() #RunningStatsNormalizer()
    config.reward_normalizer = RewardRunningStatsNormalizer() #RewardRunningStatsNormalizer()
    config.discount = 0.99
    config.use_gae = True
    config.gae_tau = 0.97
    config.entropy_weight = 5e-3 #0.75
    config.rollout_length = 512*10

    config.optimization_epochs = 16
    config.num_mini_batches = 160
    config.use_full_batch = False        # This will use full batch instead of mini batching
...
def metaworld_ppo(name, args, shell_config):
    ...
    task_fn = lambda log_dir: ContinualWorld(name=name, env_config_path=env_config_path, log_dir=log_dir)
    ...
    eval_task_fn= lambda log_dir: ContinualWorld(name=name, env_config_path=env_config_path, log_dir=log_dir)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--shell_config_path', help='shell config', default='./shell_configs/the_chosen_one/metaworld.json')                         # File path to your chosen shell.json configuration file. Changing the default here might save you some time.
    ...
    if shell_config['env']['env_name'] == 'continualworld':
        name = Config.ENV_CONTINUALWORLD
        if args.eval:
            metaworld_ppo_eval(name, args, shell_config)

        else:
            metaworld_ppo(name, args, shell_config)
