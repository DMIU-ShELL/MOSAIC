...
def main(args):
    ...
    mypaths = {
        'full comm' : ['log/full_comm/seed1/MetaCTgraph-shell-dist-ct28_md_eval_full-seed-9183/agent_26/240209-150346/',
                        'log/full_comm/seed2/MetaCTgraph-shell-dist-ct28_md_eval_full-seed-2745/agent_26/240210-124604'],

        'no comm' : ['log/no_comm/seed1/MetaCTgraph-shell-dist-ct28_md_eval_full-seed-9183/agent_26/240209-164032',
                    'log/no_comm/seed2/MetaCTgraph-shell-dist-ct28_md_eval_full-seed-2745/agent_26/240210-155928']
    }
    ...
    for name, p in mypaths.items():
        ...
        data['icr']['C3L ' + name]['ydata_cfi'] = np.std(shell_tpot, axis=0)
        data['icr']['C3L ' + name]['plot_colour'] = 'green'

        maximum_icr_ = max(maximum_icr_, np.max(data['icr']['C3L ' + name]['ydata']))

        # tpot
        data['tpot']['C3L ' + name] = {}
        data['tpot']['C3L ' + name]['xdata'] = np.arange(num_evals)
        data['tpot']['C3L ' + name]['ydata'] = np.mean(shell_tpot, axis=0) # average across seeds
        data['tpot']['C3L ' + name]['ydata_cfi'] = np.std(shell_tpot, axis=0)
